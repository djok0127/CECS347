...
void PortF_Init(void){ volatile unsigned long delay;
  ...
  GPIO_PORTF_CR_R = 0x02;         // allow changes to PF4,1,0
  GPIO_PORTF_DIR_R &= ~0x00;    // (c) make PF4,0 in (built-in button)
  GPIO_PORTF_AFSEL_R &= ~0x2;  //     disable alt funct on PF4,1,0
  GPIO_PORTF_DEN_R |= 0x02;     //     enable digital I/O on PF4,1,0
  GPIO_PORTF_PCTL_R &= ~0x00000000; //  configure PF4,0 as GPIO
  GPIO_PORTF_AMSEL_R &= ~0x02;  //     disable analog functionality on PF4,1,0
  GPIO_PORTF_PUR_R |= 0x00;     //     enable weak pull-up on PF4,0
...
void SysTick_Init(void){
  ...
  NVIC_ST_RELOAD_R = 16000;  // maximum reload value
